...
export function ChatWindow({
    ...
    return (
        <>
            ...
            <div className="md:hidden fixed inset-0 bg-background z-[100] flex flex-col">
                {/* Header */}
                <div className="p-4 bg-primary text-primary-foreground flex items-center justify-between shadow-md">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center overflow-hidden">
                            {recipientAvatar ? (
                                ...
                                <span className="font-bold text-base">{recipientName.charAt(0)}</span>
                            )}
                        </div>
                        <div>
                            <h4 className="font-bold text-base">{recipientName}</h4>
                    ...
                    <Button variant="ghost" size="icon" className="h-10 w-10 hover:bg-white/20 text-primary-foreground" onClick={onClose}>
                        <X className="w-5 h-5" />
                    </Button>
                </div>

                {/* Messages - with safe area for mobile bottom nav */}
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-muted/30 pb-safe">
                ...
                <div className="p-4 bg-background border-t border-border pb-safe">
                    <form onSubmit={handleSend} className="flex gap-2">
                        <Input
                            ...
                            className="flex-1 rounded-full bg-muted border-none focus-visible:ring-2 h-11 px-4"
                            autoFocus
                        />
                        <Button
                            type="submit"
            ...
            <div className="hidden md:flex fixed bottom-4 right-4 w-96 bg-background border border-border rounded-2xl shadow-2xl flex-col z-50 h-[600px] max-h-[85vh]">
                ...
                {/* Input */}
                <div className="p-3 bg-background border-t border-border rounded-b-2xl">
                    <form onSubmit={handleSend} className="flex gap-2">
                        <Input
                            value={newMessage}
                            onChange={(e) => setNewMessage(e.target.value)}
